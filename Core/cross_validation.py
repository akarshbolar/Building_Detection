...
def indices_build(y, k_fold, seed):
    """
...
def cross_validation_loop(model, Y, X, k_indices, k):
    """
    ...
    img_patches_groundtruth = make_patch_groundtruth(y_te, 16, 16)
    y_real = np.mean(img_patches_groundtruth, axis=(1, 2)) > 0.25
    
    return get_result(y_real, Z)    
...
def k_fold_cross_validation(model, Y, X, k_fold, seed):
    ...
    """
    np.random.seed(seed)
    k_indices = indices_build(Y, k_fold, seed)
    results = np.zeros(k_fold)
    for k in range(k_fold):
        results[k] = cross_validation_loop(model, Y, X, k_indices, k)
        print('Accuracy: ' + str(results[k]))
    print(results)
    print('Cross validation accuracy: ' + str(np.mean(results)) + ', std=' + str(np.std(results)))
    
def quick_cross_validation(model, Y, X, k_fold, seed):
    """
    ...
    np.random.seed(seed)
    k_indices = indices_build(Y, k_fold, seed)
    result = cross_validation_loop(model, Y, X, k_indices, 0)
